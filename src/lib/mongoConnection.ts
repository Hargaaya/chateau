...
async function mongoConnection() {
  if (!connectionString || connectionString === undefined) {
    throw new Error("Please define the MONGO_CONNECTION_STRING environment variable in a .env.local file.");
  }
  ...
  if (!cachedClient.promise) {
    cachedClient.promise = mongoose.connect(connectionString).then((client) => client);
